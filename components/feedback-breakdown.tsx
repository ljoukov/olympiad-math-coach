...
export function FeedbackBreakdown({
  ...
  const markColor =
    estimatedMarks >= 7 ? "text-green-700 bg-green-50 border-green-200" :
    estimatedMarks >= 4 ? "text-amber-700 bg-amber-50 border-amber-200" :
  ...
  return (
    <div className="space-y-6">
      <div className={`rounded-xl border-2 p-6 text-center ${markColor}`}>
        ...
        <p className="text-5xl font-bold">{estimatedMarks}<span className="text-2xl">/10</span></p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {rubricBreakdown.map((item, i) => {
            ...
            const Icon = ratio >= 0.8 ? CheckCircle2 : ratio >= 0.4 ? AlertCircle : XCircle
            const color = ratio >= 0.8 ? "text-green-600" : ratio >= 0.4 ? "text-amber-600" : "text-red-500"
