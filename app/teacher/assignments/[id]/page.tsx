...
type AssignmentDetailResponse = {
  assignment: { id: string; title: string; dueAt: string; problemIds: string[] }
  problems: {
    id: string
    title: string
    difficulty: number
...
const fetcher = (url: string) => authedJsonFetch<AssignmentDetailResponse>(url)

export default function AssignmentDetailPage() {
  const { user, isLoading: authLoading } = useRequireAuth()
  const { id } = useParams<{ id: string }>()
  const { data, isLoading } = useSWR<AssignmentDetailResponse>(
    user ? `/api/teacher/assignments/${id}` : null,
    fetcher
  )
